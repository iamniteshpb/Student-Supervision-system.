...
if(isset($login))
 {
	...
	$pass=$_POST['pass'];
//  $pass=md5($pass);
	$que=mysqli_query($con,"select email,pass,status from supervisor where email='$Email' and pass='$pass'");
	 ...
	 if($row)
	 {
		...
		if($status['status']==1)
		{
		$err="<font color='red'>Your account activation is Pending Pls Contact to Admin </font>";
		}
		else
		{
		$_SESSION['supervisior']=$Email;
